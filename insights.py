...
# ── Helper: strip non-latin-1 characters safely ─────────────────────────────
def safe(text):
    """Replace any character that cannot be encoded in latin-1."""
    return (text
            .replace('\u2013', '-')   # en dash
            .replace('\u2014', '--')  # em dash
...
def plot_insights_chart(df, model, X):
    ...
    plt.close()
    print(f"[OK] Insights chart saved -> {path}")
...
def generate_pdf_report(acc):
    ...
    pdf.cell(0, 12,
             safe('Airline Customer Experience - Insights Report'),
             ln=True)
    pdf.set_font('Arial', '', 11)
    pdf.cell(0, 8, safe(f'Model Accuracy: {acc*100:.2f}%'), ln=True)
    pdf.ln(4)

    # ── Key Findings ─────────────────────────────────────────
    pdf.set_font('Arial', 'B', 13)
    pdf.cell(0, 10, safe('Key Findings'), ln=True)
    pdf.set_font('Arial', '', 11)
    findings = [
    ...
    for f in findings:
        pdf.multi_cell(0, 8, safe(f'  * {f}'))
    pdf.ln(4)

    # ── Recommendations ──────────────────────────────────────
    pdf.set_font('Arial', 'B', 13)
    pdf.cell(0, 10, safe('Recommendations'), ln=True)
    for area, tip in RECOMMENDATIONS:
        pdf.set_font('Arial', 'B', 11)
        pdf.cell(0, 8, safe(f'  {area}'), ln=True)
        pdf.set_font('Arial', '', 11)
        pdf.multi_cell(0, 7, safe(f'    {tip}'))
    pdf.ln(4)
    ...
    for img_name, caption in [
        ...
        if os.path.exists(img_path):
            ...
            pdf.cell(0, 10, safe(caption), ln=True)
            pdf.image(img_path, x=15, w=180)

    report_path = REPORTS_DIR + 'airline_insights_report.pdf'
    os.makedirs(REPORTS_DIR, exist_ok=True)
    pdf.output(report_path)
    print(f"\n[OK] PDF report saved -> {report_path}")
